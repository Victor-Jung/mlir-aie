...
int main(int argc, const char *argv[]) {
  ...
  int A_VOLUME = heads * M * K;
  int B_VOLUME = heads * N * K;
  int C_VOLUME = heads * M * N;
  ...
  golden_reference_verification::load_golden_inputs(AVec, BVec);
  if (verbosity >= 1) {
    golden_reference_verification::print_golden_reference_info();
    std::cout << "Loaded golden reference inputs:" << std::endl;
    std::cout << "  A[0] = " << (int)AVec[0] << ", A[1] = " << (int)AVec[1] << std::endl;
    std::cout << "  B[0] = " << (int)BVec[0] << ", B[1] = " << (int)BVec[1] << std::endl;
  }
  ...
  int errors = 0;
  float macs = 2.0 * float(heads) * float(M) * float(K) * float(N);
  ...
  for (unsigned iter = 0; iter < num_iter; iter++) {
    ...
    if (do_verify) {
      ...
      errors = golden_reference_verification::verify_against_golden<A_DATATYPE, C_DATATYPE, ACC_DATATYPE>(
          CVec, verbosity, abs_tol, rel_tol);
