...
// ----------------------------------------------------------------------------
int verify_against_golden(const std::vector<float>& actual, int verbosity) {
    int errors = 0;
    for (int i = 0; i < N; ++i) {
...
int main(int argc, const char *argv[]) {
  ...
  std::vector<INOUT0_DATATYPE> AVec(INOUT0_VOLUME);
  for (int i = 0; i < INOUT0_VOLUME; i++) {
    AVec[i] = static_cast<INOUT0_DATATYPE>(golden_input[i]);
  }
  INOUT0_DATATYPE *bufInOut0 = bo_inout0.map<INOUT0_DATATYPE *>();
  memcpy(bufInOut0, AVec.data(), AVec.size() * sizeof(INOUT0_DATATYPE));
  ...
  for (unsigned iter = 0; iter < num_iter; iter++) {
    ...
    if (do_verify) {
      ...
      std::cout << "First 10 values of A (input):" << std::endl;
      for (int i = 0; i < 10; ++i)
          std::cout << static_cast<float>(AVec[i]) << " ";
      std::cout << std::endl;      

      std::vector<float> actual(BVec.size());
      for (size_t i = 0; i < BVec.size(); ++i)
        actual[i] = static_cast<float>(BVec[i]);
      errors = verify_against_golden(actual, verbosity);

      std::cout << "First 10 values of B (output):" << std::endl;
      for (int i = 0; i < 10; ++i)
          std::cout << static_cast<float>(BVec[i]) << " ";
      std::cout << std::endl;

      std::cout << "First 10 values of golden B (reference):" << std::endl;
      for (int i = 0; i < 10; ++i)
