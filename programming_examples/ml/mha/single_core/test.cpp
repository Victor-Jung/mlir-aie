...
int main(int argc, const char *argv[]) {
  ...
  auto bo_qk =
      xrt::bo(device, QK_SIZE, XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(6));
  // auto bo_a =
  //       xrt::bo(device, QK_SIZE, XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(7));

  // auto bo_tmp1 = xrt::bo(device, 1, XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(7)); // VJUNG: TODO: Check if we can remove this
  ...
  std::vector<DTYPE_IN> VVec(V_VOLUME);
  ...
  for (unsigned iter = 0; iter < num_iter; iter++) {
    ...
    auto run = kernel(opcode, bo_instr, instr_v.size(), bo_q, bo_k, bo_qk, bo_trace);
