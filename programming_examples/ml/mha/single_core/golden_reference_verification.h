...
int verify_against_golden(const std::vector<Tout>& C, int verbosity = 0, 
    ...
    for (int head = 0; head < golden_reference::HEADS; head++) {
        for (int row = 0; row < golden_reference::S_q; row++) {
            for (int col = 0; col < golden_reference::S_kv; col++) {
                ...
                Tout expected = (Tout)golden_reference::QK[idx];
                Tout actual = C[idx];
    ...
    if (n_errors > 0 && verbosity >= 1) {
        ...
        std::vector<Tout> golden_vec(golden_reference::QK.begin(), 
                                   golden_reference::QK.end());
        matmul_common::print_matrix(golden_vec, golden_reference::S_kv);
        std::cout << std::endl << "Actual Output:" << std::endl;
        matmul_common::print_matrix(C, golden_reference::S_kv);
    }
