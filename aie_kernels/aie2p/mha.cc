...
extern "C" {
    ...
    void matmul_PV(bfloat16 *Q, bfloat16 *K, bfloat16 *out, bfloat16 *scale_buffer, const int32_t S_q, const int32_t S_kv) {
        
        // VJUNG: Scale O_{i-1} by 1/exp(m_{i-1} - m_{i})
        const int elem_iters = S_q / VEC_LEN;


        for (int32_t i = 0; i < S_q * S_kv; i++) {
            out[i] = out[i] * 1.0f; // VJUNG: TODO
        }
            ...
            for(int32_t k = 0; k < 4; k++){ // Iterate for 4 rows 
                for(int32_t j = 0; j < 2; j++){ // Each row is broken down into 2 blocks of 8
                    for (int32_t i = 0; i < 8; i++) {
                        out[i + j*32 + k*8 + l*64] = out[i + j*32 + k*8 + l*64] * aie::inv(scale_buffer[2*S_kv + (k + l*4)]);
                    }
                }
            }
        }
